...
describe('#doSearch()', () => {
  ...
  it('recognizes multiple languages', async () => {
    const result = await searchHandler('注入', config);
    expect(result).to.have.length(1);
  });

  it('recognizes multiple languages in one sentence', async () => {
    const result = await searchHandler('天擎', config);
    expect(result).to.have.length(1);
  });
  ...
  // it('cut sentense', async () => {
  //   var result = await nodejieba.cut('当上CEO，走上人生巅峰');
  //   console.log(result);
  //   /* eslint-disable no-unused-expressions */
  //   !expect(result).to.be.not.null;
  // });
